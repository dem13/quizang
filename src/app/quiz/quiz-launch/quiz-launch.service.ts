...
export class QuizLaunchService {
  ...
  async initializeGameSettings(launchSettings: LaunchSettings, playerNames: string[]): Promise<GameSettings> {
    if (!launchSettings.roundsAmount || !launchSettings.playersAmount) {
      throw new Error('Invalid launch settings.');
    }

    const questions = await this.generateQuestions(launchSettings.roundsAmount, launchSettings.playersAmount);

    return {
      ...launchSettings,
      questions,
      players: playerNames.map((name, i) => new Player(
        this.generateNameIfEmpty(name, i),
        this.generatePlayerId(name, i),
        launchSettings.livesAmount || 3
      )),
    }
  }
  ...
  private async generateQuestions(roundsAmount: number, playersAmount: number): Promise<Question[]> {
    ...
    return response.results.map(result => new Question(
